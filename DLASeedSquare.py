# DLA Including a square seed, George Richards 4228068
import numpy as np
...
def initialise(length, movements):
    # Seed particle is built from a square with width 20, 'sticky' sites will
    # as usual be all sites adjacent
    lattice = np.zeros((length+1,length+1))
    widthSquare = 10
    for y in range(2*widthSquare):
        for x in range(2*widthSquare):
            lattice[int((length/2)-widthSquare+y),int((length/2)-widthSquare+x)] = 1
