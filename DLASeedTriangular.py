# DLA Including a triangular seed, George Richards 4228068
import numpy as np
...
def initialise(length, movements):
    # Seed particle is built from a triangle, 'sticky' sites will as usual 
    # be all sites adjacent
    lattice = np.zeros((length+1,length+1))
    triangleBase = 18
    triangleRows = np.linspace(2,triangleBase,triangleBase/2)
    for n in range(int(triangleBase/2)):
        lattice[int(length/2 + triangleBase/4 - n), int((length/2)-triangleRows[n]/2):int((length/2) + triangleRows[n]/2 + 1)] = 1
    lattice[int(length/2 + triangleBase/4 + 1), int(length/2)] = 1
...
plt.title('Fractal: Triangular Seed')
