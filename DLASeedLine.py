# DLA Including a line seed, George Richards 4228068
import numpy as np
...
def initialise(length, movements):
    # Seed particle is built from a line with width 40, 'sticky' sites will
    # as usual be all sites adjacent
    lattice = np.zeros((length+1,length+1))
    lineWidth = 40
    lattice[int(length/2), int((length/2)-(lineWidth/2)):int((length/2)+(lineWidth/2)+1)] = 1
